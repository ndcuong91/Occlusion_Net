...
class COCODemo(object):
    ...
    def overlay_keypoints_graph(self, image, predictions, color, target='car'):
        ...
        for region in kps:
            if target == 'car':
                if color == 'occlusion':
                    image = vis_keypoints_car_vis(image, region.transpose((1, 0)),kp_thresh = 10)
                    image = vis_keypoints_car_invis(image, region.transpose((1, 0)),kp_thresh = 10)
                if color == 'rainbow':
...
def vis_keypoints_car_vis(img, kps, kp_thresh=2, alpha=1):
...
## Beautiful colors made by zensors
    colors[3] = (0,0,255)
    colors[0] = (0,255,0)
    colors[1] = (224,123,153) # purple
    colors[4] = (255,128,0)
    colors[2] = (34,194,244)
## colors to show occlusions
    colors[3] = (0,255,0)
    colors[0] = (0,255,0)
    colors[1] = (0,255,0) # purple
    colors[4] = (0,255,0)
    colors[2] = (0,255,0)
...
def vis_keypoints_car_invis(img, kps, kp_thresh=2, alpha=0.5):
    """Visualizes keypoints (adapted from vis_one_image).
    kps has shape (4, #keypoints) where 4 rows are (x, y, logit, prob).
    """
    dataset_keypoints = VehicleKeypoints.NAMES
...
def vis_keypoints_person(img, kps, kp_thresh=2, alpha=0.7):
    """Visualizes keypoints (adapted from vis_one_image).
    kps has shape (4, #keypoints) where 4 rows are (x, y, logit, prob).
    """
